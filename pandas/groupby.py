...
result=df.groupby(["Department", "City"]).groups # Group by Department and City
...
result=df.groupby("Department")["Salary"].std() # Standard Deviation of the groups
...
result=df.groupby("Department")["Salary"].agg(["sum", "mean", "max", "min", "count", "std", "var"]) # Aggregate the groups
result=df.groupby("Department")["Age"].agg(["sum", "mean", "max", "min", "count", "std", "var"]).loc["IT"] # Aggregate the groups of IT
result=df.groupby("Department")["Age"].agg(["sum", "mean", "max", "min", "count", "std", "var"]).loc["IT"]["mean"] # Aggregate the groups of IT and get the mean
print(result)
